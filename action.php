...
if(isset($_POST["action"]))
{
 if($_POST["action"] == "fetch")
 {
  ...
  <table class="table table-bordered table-striped">
   <tr>
    ...
    <th>Просмотр загруженного файла</th>
   </tr>
   ';
  if(count($folder) > 0)
  {
   foreach($folder as $name)
   {
    $output .= '
     <tr>
      ...
      <td><button type="button" name="upload" data-name="'.$name.'" class="upload btn btn-info btn-xs">Загрузить файл
</button></td>
      <td><button type="button" name="view_files" data-name="'.$name.'" class="view_files btn btn-default btn-xs">Просмотр файлов</button></td>
     </tr>';
   }
  }
  else
  {
   $output .= '
    <tr>
     <td colspan="6">Не найдено папок</td>
    </tr>
   ';
  }
  $output .= '</table>';
 ...
 if($_POST["action"] == "change")
 {
  if(!file_exists($_POST["folder_name"]))
  {
   ...
   echo 'Имя папки изменено';
  }
  else
  {
   echo 'Имя папки не изменено';
  }
 }
 ...
 if($_POST["action"] == "change_file_name")
 {
  ...
  {
   echo 'Имя файла изменено';
  }
  else
  {
   echo 'Имя файла не изменено';
  }
 }
}
?>
